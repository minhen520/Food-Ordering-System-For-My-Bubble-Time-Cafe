            ...
            <div class="m-50">
                ...
                if (isset($_POST['search'])) {
                    if (!empty($_POST['search'])) {
                        ...
                        $sql = "SELECT * FROM Menu WHERE item_category LIKE '%$search%' OR item_name LIKE '%$search%' OR item_id LIKE '%$search%' ORDER BY item_id;";
                    } else {
                ...
                if ($result = mysqli_query($link, $sql)) {
                    if (mysqli_num_rows($result) > 0) {
                        ...
                        while ($row = mysqli_fetch_array($result)) {
                            ...
                            echo '<td><center><img src="../menuCrud/Res_img/' . $row['item_img'] . '" class="img-responsive radius" style="max-height:100px;max-width:150px;" /></center></td>';
                            echo "<td>";
                            // Modify link with the pencil icon
                             $update_sql = "UPDATE Menu SET item_name=?, item_type=?, item_category=?, item_price=?, item_description=? , item_img=? WHERE item_id=?";
                            ...
                            echo '<form action="../menuCrud/updateItemStatus.php" method="POST">';
                            echo '<input type="hidden" name="item_id" value="' . $row['item_id'] . '">';
                            echo '<button type="submit" name="toggle_status" class="btn btn-sm ' . ($row['item_status'] == 1 ? 'btn-success' : 'btn-danger') . '">' . ($row['item_status'] == 1 ? 'Available' : 'Unavailable') . '</button>';
                            echo '</form>';
