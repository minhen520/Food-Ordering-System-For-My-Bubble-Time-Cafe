...
if (isset($_GET['addToCart'])) {
    ...
    if ($result) {
        if (mysqli_num_rows($result) > 0) {
            ...
            $insert_kitchen_sql = "INSERT INTO Kitchen (table_id, item_id, quantity, time_submitted,kit_status) VALUES ('$table_id', '$item_id', '$quantity', '$currentTime', '$currentStatus')";
...
if (isset($_GET['confirmOrder'])) {
    $table_id = $_GET['table_id'];
    $bill_id = intval($_GET['bill_id']);
    $update_kitchen_sql = "UPDATE Kitchen SET kit_status = 1 WHERE table_id = '$table_id'";
