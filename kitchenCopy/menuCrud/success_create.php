...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($_FILES['file']['size'] > 0) {
        ...
        $insert_query = "INSERT INTO Menu (item_id, item_name, item_category, item_price, item_description, item_img) 
                        VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($insert_query);

        // Bind the parameters
        $stmt->bind_param("ssssss", $item_id, $item_name, $item_category, $item_price, $item_description,$fnew);
