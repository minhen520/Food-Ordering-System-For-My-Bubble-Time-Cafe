...
$bill_id = $_SESSION['latest_b'] ?? null;
$table_id = $_SESSION['latest_t'] ?? null;

if (!empty($_SESSION['latest_b'])) {
    header("Location: orderItem.php?bill_id=" . $_SESSION['latest_b'] . "&table_id=" . $_SESSION['latest_t']);
    exit; 
}
                ...
                <div class="row justify-content-center">
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        if ($confirm_status_result && mysqli_num_rows($confirm_status_result) > 0) {
                            ...
                            if ($confirm_time_row['kit_status'] == 1) {
                                $kitchen_cook = true;
                            }
                        }
                        ...
                        if ($latestBillData) {
                            ...
                            if ($result2 && mysqli_num_rows($result2) > 0 || $kitchen_cook == true) {
                    ...
                    echo '<a href="javascript:void(0);" onclick="checkTable(' . $table_id . ',' . $latestBillID . ')" class="btn btn-primary btn-block btn-lg" style="color:black; '
                    ...
                    . 'Table: ' . $table_id /* . '<br>Bill ID: ' . $latestBillID */. '<br>Capacity: ' . $capacity;
                        ...
                        echo '<a href="javascript:void(0);" onclick="checkTable(' . $table_id . ',' . $latestBillID . ')" class="btn btn-primary btn-block btn-lg" '
...
<script>
        function checkTable(clickedTableID, latestBillID) {
            var userInput = prompt("Please write down your table number:");
            if (userInput == clickedTableID) {

                window.location.href = "orderItem.php?bill_id=" + latestBillID + "&table_id=" + clickedTableID;
            } else {
                alert("You are required to sit at table " + clickedTableID + " and order at that table.");
            }
        }
    </script>
