...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($check_payment_result) {
        ...
        if ($row['card_id'] !== null) {
            ...
            $insert_card_sql = "INSERT INTO card_payments (account_holder_name, card_number, expiry_date) 
                                VALUES (?, ?, ?)";
            
            $stmt = $link->prepare($insert_card_sql);
            $stmt->bind_param("sss", $account_holder_name, $card_number, $expiry_date);
            ...
            if ($stmt->execute()) {
                ...
                if (!empty($member_id)) {
                    ...
                    $_SESSION['logged_customer_points'] += $points;
                    if ($stmt->execute()) {
                    ...
                    echo "You're losing your points! Fast Register as member";

                    $update_bill_sql = "UPDATE Bills SET card_id = ?, payment_method = ?, payment_time = ?,
                staff_id = 0, member_id = 0, reservation_id = 0 WHERE bill_id = ?";

                $stmt = $link->prepare($update_bill_sql);
                $payment_method = "card";
                $stmt->bind_param("issi", $card_id, $payment_method, $currentTime, $bill_id);
                }
                unset($_SESSION['memberId']); // Unset after successful update
