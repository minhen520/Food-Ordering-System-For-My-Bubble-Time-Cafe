...
if (isset($_GET['addToCart'])) {
    ...
    $currentStatus = 0; //not yet confirm order
    ...
    if ($result) {
        if (mysqli_num_rows($result) > 0) {
            ...
            $insert_kitchen_sql = "INSERT INTO Kitchen (bill_id, table_id, item_id, quantity, time_submitted,remarks,kit_status) VALUES ('$bill_id', '$table_id', '$item_id', '$quantity', '$currentTime', '$remark','$currentStatus')";
...
if (isset($_GET['confirmOrder'])) {
    ...
    $bill_id = intval($_GET['bill_id']);
    $item_id = $_GET['item_id'];
    $update_kitchen_sql = "UPDATE Kitchen SET kit_status = 1 WHERE table_id = '$table_id' AND bill_id = '$bill_id'";
    if (mysqli_query($link, $update_kitchen_sql)) {
        echo '<script>alert("Item added to cart successfully")</script>';
        header("Location: orderItem.php?bill_id=" . urlencode($bill_id) . "&table_id=" . $table_id . "&add_order=true");
    } else {
        echo '<script>alert("Error adding item to cart: ' . mysqli_error($link) . '")</script>';
    }
}
